...
function imgui.OnDrawFrame()
	if show_menu.v then
		...
		if show_info.v then
			...
			imgui.Begin(u8'Информация', show_info, imgui.WindowFlags.AlwaysAutoResize)
				...
				imgui.Text(u8'Отдельное спасибо ютуберу "Овсянка GameZ" за поддержку =)')
				imgui.Text(' ')
		...
		if show_nastrouka.v then
			...
			imgui.Begin(u8'Настройки', show_nastrouka, imgui.WindowFlags.AlwaysAutoResize)
				-- SetStyle.Style.StateStyle == 1
				if imgui.Button(u8'Стандартный стиль', imgui.ImVec2(190.0, 30.0)) then
					StandartTheme()
					SetStyle.Style.StateStyle = 5
					ini.save(SetStyle)
				end
				if imgui.Button(u8'Вишнёвый стиль', imgui.ImVec2(190.0, 30.0)) then
					CherryTheme()
					SetStyle.Style.StateStyle = 6
					ini.save(SetStyle)
				end
				if imgui.Button(u8'Светло-Серая тема', imgui.ImVec2(190.0, 30.0)) then
					black_grey()
					SetStyle.Style.StateStyle = 4
					ini.save(SetStyle)
				end 
				if imgui.Button(u8'Темно-Оранжевая тема', imgui.ImVec2(190.0, 30.0)) then 
					temno_oransh()
					SetStyle.Style.StateStyle = 3
					ini.save(SetStyle)
				end 
				if imgui.Button(u8'Темно-Красная тема', imgui.ImVec2(190.0, 30.0)) then
					temnokrasn()
					SetStyle.Style.StateStyle = 2
					ini.save(SetStyle)
				end 
				if imgui.Button(u8'Темно-Бордовая тема', imgui.ImVec2(190.0, 30.0)) then
					temnobordovui()
					SetStyle.Style.StateStyle = 1
					ini.save(SetStyle)
				end 
				...
				if show_overlay.v then
					...
					-- imgui.Checkbox(u8'Дата и время', show_overlay_time)
				end
			imgui.End()
		end
	...
	-- Оверлей --
	if show_overlay.v == true and show_overlay_fps.v or show_overlay.v == true and show_overlay_koor.v or show_overlay.v == true and show_overlay_healh.v then
		-- local sw, sh = getScreenResolution()
		imgui.SetNextWindowPos(imgui.ImVec2(SetStyle.Kor_OverLay.x, SetStyle.Kor_OverLay.y), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
